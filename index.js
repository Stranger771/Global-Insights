...
let combineContainer = document.querySelectorAll('.otherInfo, .location-container')
let wrapper = []
...
function renderSearch(country, search){
    let filterArray = country.filter((item)=>{
        return item.name && item.name.common.toLowerCase().includes(search.searchText.toLowerCase())
    })
...
let introElements2 = document.querySelectorAll('.intro-description .style-second-heading span');
function repeat2() {
...
let person = new userFeedBack('EchoEnigma', 'Great experience with Global Insights!', 'Images/download3.jpg')
let person2 = new userFeedBack('QuantumPioneer', 'I love the informative content.', 'Images/The hard life.jpg')
let person3 = new userFeedBack('NovaNinja', 'Highly recommended for travelers.', 'Images/download (1).jpg')
